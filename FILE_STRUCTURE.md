...
video-scraper（GitHub リポジトリ） ├── main ブランチ │ ├── .gitignore │ ├── README.md │ ├── requirements.txt │ └── \[その他の本番コード] │ └── ai-dev ブランチ ├── video_downloader.py          # ★Step 0: 動画自動ダウンロードエンジン（新規） ├── run\_antigravity.bat ...................... \[実行エントリーポイント]      ├── master_batch_refiner.py ................. [コアエンジン Step1-4]
      ├── grand_master_integrator.py .............. [統合推論エンジン Step5]
      ├── total_evidence_integrator.py ............ [神経ネットワーク構築エンジン Step6]
      ├── config_params.json ....................... [パラメータ設定] ├── system\_architecture.md .................. \[技術仕様書] ├── instructions.md .......................... \[プロンプト仕様] ├── DEVELOPMENT\_GUIDELINES.md ............... \[開発運用ガイド] ├── SYSTEM\_OVERVIEW.md ....................... \[本ドキュメント] ├── FILE\_STRUCTURE.md ........................ \[ファイル構成ガイド] └── batch\_refine\_work/（実行時生成） ├── frames\_03/ ├── frames\_04/ └── ...
...
| `video_downloader.py` | Python スクリプト | Step 0: UTAGEコースからレッスンURL収集・yt-dlpでMP4ダウンロード | ai-dev | 毎回実行 |

| `master_batch_refiner.py` | Python スクリプト | 動画1本ずつを処理（JSON + DB生成）| ai-dev | 毎回実行 |
| `grand_master_integrator.py` | Python スクリプト | 全JSONを統合・推論し高次知識を生成するエンジン | ai-dev | 全動画処理完了後 |
| `total_evidence_integrator.py` | Python スクリプト | 全DBを統合し論理ネットワークを構築するエンジン | ai-dev | grand_master完了後 |
...
| `Mk2_OCR_{lecture_id}.txt` | テキスト | 中間出力：Mud フィルタ済み視覚テキスト | master_batch_refiner.py | デバッグ・検証用 |
| `Mk2_Grand_Master_Logic.json` | JSON | 全動画の知識を統合・推論した最終知識ファイル | grand_master_integrator.py | 知識統合データベース |
| `Mk2_Total_Evidence.db` | SQLite3 | 全エビデンスを統合した論理ネットワークDB（evidence_index + logical_network テーブル） | total_evidence_integrator.py | 論理ネットワーク検索・分析 |
...
| `video_list.txt` | テキスト | Step0生成: タイトル###URL形式のレッスンリスト | gitignore対象 |

| `logs/failed_downloads.txt` | テキスト | Step0生成: ダウンロード失敗リスト | エラー確認用 |
...
run_antigravity.bat
  ↓ (起動)
video_downloader.py                  ← Step 0：UTAGE → downloaded_videos/
  ↓ (全動画ダウンロード後)
master_batch_refiner.py              ← Step 1-4：動画1本ずつ処理
  ├─→ config_params.json (パラメータ参照)
  ├─→ Whisper モデル（音声文字起こし）
  ├─→ EasyOCR（フレームOCR）
  ├─→ Gemini API（知識抽出 OFLOOP）
  ├─→ FFmpeg（フレーム抽出）
  └─→ SQLite3（Sidecar DB 生成）
        ↓ (全動画処理完了後)
grand_master_integrator.py           ← Step 5：全JSON統合・推論
  ├─→ Mk2_Core_*.json（全21本を読み込み）
  ├─→ Gemini API（高次推論・STRATEGY生成）
  └─→ Mk2_Grand_Master_Logic.json（出力）
        ↓
total_evidence_integrator.py         ← Step 6：全DB統合・論理ネットワーク構築
  ├─→ Mk2_Sidecar_*.db（全21本をマージ）
  ├─→ Mk2_Grand_Master_Logic.json（参照）
  ├─→ Gemini API（論理リレーション推論）
  └─→ Mk2_Total_Evidence.db（出力：evidence_index + logical_network）
...
Q: Gemini API のモデル ID を変更したい
A: .env ファイルの GEMINI_MODEL_ID を変更してください。
   3ファイル（master_batch_refiner.py / grand_master_integrator.py /
   total_evidence_integrator.py）すべてに自動で反映されます。
   コードの修正は不要です。
